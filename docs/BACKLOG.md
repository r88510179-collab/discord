...
- **Sanity-guard telemetry** — `EVENT_DATE_SANITY_MODE` (off/shadow/enforce, unset=off byte-identical): the ALWAYS-ON #153/#154 gap guard now reports each rejection as an `event_date_sanity_rejected` pipeline event under shadow/enforce (flag gates telemetry only; see docs/FLAG-FLIPS.md).
- **Ingest threading, the cheap reliable wins** — twitter-handler now passes the vision-extracted `event_date` through its pick rebuild + all three `createBetWithLegs` payloads (twitter_vision was the largest bucket at 1376 rows / 0 populated — pure drop-on-the-floor); warRoom `war_split` singles inherit the parent's `event_date` (both branches); `ocrFirstWiring.ocrBetToInternalBets` passes the Groq-captured first leg `start_time` verbatim (dormant until OCR_FIRST cutover); `parseBetSlipImage`'s prompt (live again via 🔄 re-ingest #182 + Onyx reclassify #179) gained the EVENT DATE/TIME verbatim-copy directive + example field it never had. All values still route through the write gate (verbatim string in, deterministic resolution + gap guard at insert).
