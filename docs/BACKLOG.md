...
- **#3 — event-aware retry.** Distinguish "event is in the future" from "event just finished" instead of the current fixed 30-min recheck + 15-attempt cap. The flat cap churns `GRADE_TOO_RECENT` on bets whose game hasn't happened yet (burns attempts) while being slow to retry bets that just went final. Make the recheck interval/attempt budget event-time aware.
