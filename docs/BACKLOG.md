...
### Duplicate migration 006 — both `ADD COLUMN season`
Duplicate migration 006 (`006_add_season_to_bets.sql` + `006_add_season_column.sql`) both `ADD COLUMN season` — reconcile. The migrator runs both (tracks by filename); the second throws `duplicate column name`, swallowed by the migrator's duplicate-column tolerance (`services/migrator.js:61-71`) — that is what masks it. The `database.js:62` boot guard is a separate no-op fallback (runs *after* migrations, when the column already exists), NOT the masker. See `docs/SEASON-RESET.md`.
