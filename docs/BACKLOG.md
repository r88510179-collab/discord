...
- **#3 — event-aware retry.** ✅ **SHIPPED in `shadow` (#124, `3269ab4`, v691, 2026-06-18)** — `nextAttemptForEvent` + `EVENT_AWARE_RECHECK` flag now distinguish "event is in the future" from "event just finished" instead of the flat 30-min recheck; see the shipped entry under "Retry storm … RESOLVED" and CODEMAP §grading.js. **Resolved + live:** `EVENT_AWARE_RECHECK=enforce` since ≈2026-07-04; the collision blocker was resolved by POST_EVENT_SWEEP_SETTLE_MS (#194, 2026-07-08 — flip pre-dated the fix by ~4 days, window scanned clean; see FLAG-FLIPS 2026-07-13 retro + correction entries).
...
**SUPERSEDED 2026-05-14**: Fix A's Gemma fallback target permanently disabled via GEMMA_FALLBACK_DISABLED=true (v431, cf58b4c) — Surface Pro 5 hardware ceiling makes inference within Fly's 90s timeout infeasible (7-17min real inference times). Visibility for these drops now provided by v434 admin-log notice (`⚠️ Slip dropped` posted to ADMIN_LOG_CHANNEL_ID with View Original link). **Fix built in this PR (not deployed):** image-bearing `PURE_SLIP_CHANNEL_IDS` failures now route to the existing recoverable `MANUAL_REVIEW_HOLD` queue; text-only pure-slip noise and non-human surfaces keep their current drops. This closes the going-forward loss route without loosening the hard `is_bet === false` parser gate or auto-creating an unverified bet. Original Fix A note preserved for audit:
...
**Cross-ref — after-the-fact rescue shipped (Phase 2b-2):** for HRB shares sitting in the hold queue, on-demand **Recover** (#56, `5da6a49`; `POST /api/admin/holds/:ingestId/recover` → `holdReview.recoverHold`) re-fetches the now-unfurled message and re-runs the existing `vision_slip` extract+create path. Recovered bets are backdated to the slip post time (#59, `705db91`) and given a 3-day `sweep_exempt_until` grace window (#62, `94a973b`) so the 7-Day Sweeper can't false-LOSS them first; the recover fetch retries transient Discord misses (#65, `ad08321`). The fix built in this PR feeds going-forward image-bearing pure-slip failures into that already-shipped recovery path; it does not change the recovery API or the strict creation-time parser verdict. Mechanism mapped in docs/CODEMAP.md (§services/holdReview.js + §7-Day Sweeper + recovery grace).
...
**PR 2b (built), gated on the shadow split, which the sgp-audit validated** (docs/regrades/sgp-audit-20260710.json: zero false PASSes on live traffic; both genuine parse failures correctly FAILed): hold enrichment on a gate PASS (signed-off design D2, #41 Option A) behind **`SGP_HOLD_MODE`** (`off` default | `shadow` | `enforce`, read per call — deliberately separate from `OCR_FIRST_MODE` so the flip doesn't ride the unrelated cutover bar). At the vision-failure seam (`handlers/messageHandler.js` `trySgpDropToHold`, both `is_bet=false` and `ai_indeterminate` branches, human channels, single-image), `ocrFirstWiring.runSgpDropToHold` reruns OCR → Groq → `evaluateSgpGate`; PASS → `MANUAL_REVIEW_HOLD` carrying the OCR legs (`payload.ocrSgp`) + `ocr_sgp_hold` event; the Release modal prefills the legs and releases a real parlay (`holdReview.js` `sgpHoldPrefill`/`sgpReleasePlan`). FAIL/error emits no SGP event and falls through to the base router (generic hold for an image-bearing pure-slip failure, existing behavior elsewhere). Spec §8.5. **Follow-up once enforce is live:** remove the THROWAWAY PR 2a pulse — `services/sgpWouldHoldPulse.js`, its `healthReport.js` section, the `runSgpWouldHold` call in `runShadow`, and `tests/sgp-would-hold-pulse.test.js`.
...
### Spot-check: LOSS written pre-event despite DATE_BOUND=enforce (single instance, opened 2026-07-13)
Bet e08f34ffcbc94efc72079866f790c6a8 graded LOSS 2026-07-03 21:48:34, event_date 2026-07-03T23:15:00Z — ~87 min before scheduled start. One instance in 433 grades since 07-01; predates the EVENT_AWARE enforce flip. Likely wrong-game/wrong-time evidence or event_date drift. Action: pull grading_audit rows for this bet id and classify; no systemic pattern observed.
