...
Two test files fail on `main` independent of recent changes (surfaced during validator fix v346, 2026-04-30): `tests/migration-validation.js` (assertion mismatch on `006_add_season_*.sql` filenames) and `tests/twitter-pipeline-validation.js` (multi-pick mapping). Not blocking anything currently, but blocks the CI reliability gate from being meaningful. Triage both — likely either fix the assertions or delete the dead tests.
