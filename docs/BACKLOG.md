...
Tests: `tests/terminal-state-invariant.test.js` (38 asserts, RED-proven on all defects incl. the two review findings; wired into `npm run check` + `test:reliability`). **Cross-ref: the Stage 2 reaper (see "Stage 2 — BetService" above) remains OPEN** — this PR removes the terminal-bet churn class, NOT the stuck-pending class the reaper exists for (long-`backoff` bets with `result='pending'` are untouched). Expected observable effect on deploy: grading-side `pipeline_events` volume drops and stays down (baseline 3,333/7d on 2026-07-08, ~40% terminal churn).
...
**⚠️ REMAINS OPEN — Stage 2 reaper must inherit the same exemption predicate.** The BetService Stage 2 reaper (see "Stage 2 — BetService" above) is a future terminal-writer over stuck-pending bets; if it lands without calling the same `sweepGraceUntil`-based check (`deferVoidForGraceWindow` or the predicate directly), it re-opens exactly this hole for recovered/approved bets.

**⚠️ REMAINS OPEN — retry-cap void lacks the Build-1d adapter exemption (the WC-3 class is only HALF closed).** `shouldAutoVoidNoData` refuses to no-data-void an adapter-covered sport (`hasDeterministicAdapter`, #145), but the **retry-cap void has no such check** — and it is the writer that actually voided WC-3. Consequences today: (a) an adapter-covered soccer/WC bet that was never recovered nor war-room-approved (`sweep_exempt_until` NULL) cap-voids with zero protection after 15 gateway denials (the WC-3 bets burned 15 in ~16h–2d — parlays with prop/unsupported legs deny fast via `pending_legs`); (b) a WC-3-shaped bet re-voids on the FIRST denial after its 3-day window lapses (attempts stay ≥15; the cap check runs before anything else) unless the adapter or a human grades it first. Fix candidate: mirror the #145 exemption in the cap block (defer adapter-covered bets back to backoff; 7-day sweeper stays the backstop) — **needs design sign-off**: the cap exists to stop denial-loop churn ("denial keeps flipping state back to 'ready' faster than the backoff ladder"), so a naive sport-wide exemption could reintroduce unbounded re-pick churn for adapter sports whose bets the adapter can never settle (qualification markets, shadow-gated props inside parlays). Found during the 2026-07-08 Build-1c rebase-recheck (branch `docs/wc3-retrycap-record`).
## event_date population gap (diagnosis recovered 2026-07-08, written 2026-06-25)
Full read-only diagnosis at `docs/diagnosis/EVENT_DATE_DIAGNOSIS-2026-06-25.md` (recovered from an orphaned worktree — was never committed). Headline: event_date NULL on 96.9% of bets; only vision_slip ever populates it (11.5% hit-rate); ~17 of 91 populated values are wrong-year vision output that Gate 4 now catches at grade time. Fix is ingest-side population + write-gate year sanity + back-catalog backfill; `EVENT_DATE_SLATE=enforce` stays near-no-op until population is fixed. Interacts with: Gate 4 firings, GRADE_TOO_RECENT noise, capper-class anchoring, event-aware recheck targeting. Not yet scoped into a PR.
