...
The match-level ESPN soccer adapter (`services/sportsdata/soccer.js`, slug **fifa.world** only) is built behind `SOCCER_GRADER_MODE` (off|shadow|enforce, **default off, PR open / not deployed**). It grades team ML (3-way win), draw, double chance, FT totals, team totals, spread/handicap, BTTS; any player prop / cashout overlay / draw-no-bet falls through (props are a **second pass**, not built here).
