...
- **2026-07-02 (script shipped, operator run PENDING)** — `scripts/apply-pregate-corrections.js`: in-container correction pass for the 24 high-confidence pre-gate shadow-regrade disagreements (23 flips + `223d9043` no-selection VOID carve-out) + retro-archive of the 3 manual corrections (`3e5c01a0`/`3f78b923`/`b6065d701c`). Dry-run default (readonly), `--apply` = one transaction via `applyGradeOverride` with bankroll deps neutered (S2 fresh slate — no Beta-era bankroll writes); HARD-GATE refuses tailed bets (`user_bets` is not reconciled). Results → `docs/audits/2026-07-02-pregate-shadow-regrade.md` §Corrections applied.
