...
**Concurring non-whitelisted sources exception**: If the whitelisted source for a sport cannot be accessed (page doesn't load, not indexed, truncated) but 2+ independent non-whitelisted sources (e.g. Yahoo Sports, AP News, official team athletics pages, NBC Sports) agree verbatim on the specific stat or outcome being graded, you MAY grade the bet. Use the whitelisted source as `evidence_source` and the most precise verbatim quote from the best available source as `evidence_quote`. In `grade_reason`, explicitly note which concurring sources corroborated the outcome (e.g. "Yahoo Sports + UConn athletics + NBC Sports all confirm Karaban 3/10 from three"). This exception applies ONLY when specific-stat corroboration is independent. It does NOT relax any other rule (hallucination forbidden, quote must still contain teams/players/numbers, etc.).
