...
drifted row heals it). Drift is watched by a one-time-per-process startup log
(`logTerminalStateDriftOnce`/`countTerminalStateDrift`, `database.js`, fired on the first
`getPendingBets` call). Pinned by `tests/terminal-state-invariant.test.js`.
