...
| sweep_exempt_until | TEXT | mig 028 (Phase 2b-2) — self-expiring sweeper-grace marker. `datetime('now','+3 days')` stamped by TWO writers: `recoverHold` (recovery moment) and `approveBet` (approval moment — review-queue dwell can exceed SWEEP_DAYS). NULL only for bets that were never recovered nor war-room-approved. While set + future, the 7-Day Sweeper leaves the bet pending instead of auto-LOSS. See §7-Day Sweeper + recovery grace. **WC-3 (2026-07-08): the window is now ALSO honored by the three void-on-exhaustion writers** (retry-cap void, no-data auto-void, unscoped-sport auto-void) via `deferVoidForGraceWindow` (`services/grading.js`, same predicate as the sweeper's `sweepGraceUntil`): an active window skips the terminal void and requeues `grading_next_attempt_at` at the window's lapse, with one `GRADE_VOID_DEFERRED_EXEMPT` event (bet NOT dropped). Pre-fix the window only guarded the sweeper — three recovered HRB WC bets were no-data-voided inside it (`docs/regrades/graded_wc3_2026-07-08.json`) |
...
| idempotency_key | TEXT | mig **032** (Stage 2) — deterministic grading-side DROP dedup key `gradv1\|<bet_id>\|<grading_attempts at write>\|<stage>\|<event_type>\|<drop_reason\|\|''>` (`deriveIdempotencyKey`, `services/pipeline-events.js:155`). Populated ONLY under `PIPELINE_IDEM_MODE=enforce`; NULL for every ingest-side row, every pre-032 row, and every off/shadow write — **shadow stores the key in payload `$.idem_key` instead** so the partial unique index `idx_pipeline_events_idem_key` (`WHERE idempotency_key IS NOT NULL`) can't reject the duplicates shadow measures. NOT Gate 2 grade idempotency (mig 026) — that dedups final-grade writes; this dedups observability rows. |
...
**TERMINAL-STATE INVARIANT (2026-07-08):** every write that sets `result` to a terminal
value (`win`/`loss`/`push`/`void`) sets `grading_state='done'` in the SAME statement.
Writers, all verified: `gradeBetRecord` (`database.js` — AI finalize/sweeper/manual/war-room),
the retry-cap void (`grading.js scheduleRecheckAfterDenial` — pre-fix it wrote `'backoff'`
+ a +24h next attempt, the recurring creator of the 2026-07-08 cleanup class: 375 live
rows terminal with state `backoff`/`ready`/`quarantined`, 93% void, ~40% of 7d grading-side
`pipeline_events` churning on already-terminal bets; one-time cleanup set 372 → `done`,
snapshot `/data/state-cleanup-2026-07-08.json`), `autoVoidNoSearchableData`, the unscoped-sport
auto-void, and `applyGradeOverride` (`gradeOverride.js` — also stamps `done` so overriding a
drifted row heals it). Write-side dual: `applyBackoff`
(`grading.js`) is gated `AND result = 'pending'` so the grade loop's backoff after an
awaited AI call cannot stamp `'backoff'`/`'quarantined'` over a row a concurrent handler
(manual `/grade`, celebration/recap auto-grade) terminally graded mid-await (same
interleaving shape as the #118 grader-vs-revert race). Drift is watched by a
one-time-per-process startup log (`logTerminalStateDriftOnce`/`countTerminalStateDrift`,
`database.js`, fired on the first `getPendingBets` call; the count includes
`result='archived'` — mig 016 policy makes archived terminal, and the legacy
`!reset_season` archive must not shrink the count without healing). Pinned by
`tests/terminal-state-invariant.test.js`.
...
**`pipeline_events.stage`**: `RECEIVED`, `AUTHORIZED`, `BUFFERED`, `EXTRACTED`, `PARSED`, `VALIDATED`, `STAGED`, `DROPPED`, `MANUAL_REVIEW_HOLD`, `MANUAL_REVIEW_DISMISSED`, `MANUAL_REVIEW_RELEASED`, `PURE_SLIP_SKIP_HOLD`, `OCR_FIRST`, `RECOVERY_ATTEMPT_FAILED`, `GRADING_ENTER`, `GRADING_SEARCH`, `GRADING_AI`, `GRADING_GUARDS`, `GRADING_COMPLETE`, `GRADING_DROPPED`. Enum lives at `services/pipeline-events.js:18`. **`RECOVERY_ATTEMPT_FAILED`** (added 2026-06-12): one row per hold-recovery attempt that burned vision+OCR but yielded no bet (`validator_drop` / `no_bet_found`, incl. extract throw); `COUNT(*)` per `ingest_id` is `recoverHold`'s retry-cap counter (`RECOVERY_RETRY_CAP`=5, `services/holdReview.js`) — at the cap, un-forced recovery refuses with `recovery_exhausted` (HTTP 429) **before** the Discord fetch, so an exhausted hold costs nothing per poll; API body `{force:true}` is the operator override. Trace-only, NOT a drop (the hold stays open); absent from `pipelineHealth.EXPECTED_STAGES` like the other markers. Note: `STAGE_ENTER` etc. listed here previously were `event_type` values, not stages — those are `STAGE_ENTER`, `STAGE_EXIT`, `DROP`, `ERROR` (`EVENT_TYPES`, line 34). Write-boundary enum validation shipped as #49: `warnUnknownEnums` (`services/pipeline-events.js:142`, called at the single write boundary L169) warn-only logs non-canonical values and still writes (fire-and-forget contract preserved; closes audit F-17). **`GRADING_COMPLETE`** got its first real emitter 2026-07 (T6-01/V6): every AUTONOMOUS terminal grade write — 7-day sweep (`sweepExpiredBet`), graphic auto-grade, celebration auto-grade — now writes one `GRADING_COMPLETE`/`STAGE_EXIT` row + one `grading_audit` row via `services/grading.js emitAutonomousGradeTelemetry` (`payload.source` = `sweeper_7d`\|`graphic_auto`\|`celebration`); `GRADING_SEARCH`/`GRADING_GUARDS` remain registered-but-unemitted (T6-08).

**`pipeline_events.drop_reason`**: closed source-of-truth list is `DROP_REASONS` at `services/pipeline-events.js:43` (not enumerated here — read it there; the warn-only tripwire above flags any unregistered value). There is no separate constants module — `services/database.js` is NOT the enum home. New values must be added to that array (and CODEMAP §Enums) before a call site emits them. **`GUARD5_INSUFFICIENT_SIGNALS`** (added 2026-06-11): GUARD 5's pre-buffer signal heuristic dropped a message (`looksLikePick` <2 signals, no celebration, no images). Deliberately distinct from `PRE_FILTER_NO_BET_CONTENT` so "a real bare total was discarded by the heuristic" is queryable apart from genuine non-bet text. Only fires OUTSIDE DubClub-split channels — those bypass GUARD 5 for both webhook and human authors (incident 2026-06-11). **`VISION_RESULT_RECAP` / `VISION_UNTRACKED_WIN` / `VISION_TICKET_RECAP`** (added 2026-06-16, audit F17; registered `services/pipeline-events.js:61` / `:62` / `:63`, F17 block-comment header `:53-60`): the relay-image vision path (`handlers/messageHandler.js` `processAggregatedMessage`) classified a parse as `type:'result'` / `type:'untracked_win'` / `ticket_status:winner|loser` and `return`ed after routing to auto-grade / War-Room embed / recap-grade matching **without recording any terminal event** — 65 relay-image ingests vanished after `EXTRACTED` with zero bets and no DROP. Each branch now records the matching terminal DROP (logged before the side-effect) via the local `dropAll` closure (`handlers/messageHandler.js:995`): `VISION_RESULT_RECAP` `:1125` (before `autoGradeBet`), `VISION_UNTRACKED_WIN` `:1136` (before the War-Room `sendUntrackedWinEmbed`), `VISION_TICKET_RECAP` `:1188` (before the recap-grade matching loop). Note: only THREE new reasons — `VISION_UNTRACKED_WIN` is NOT suffixed `_RECAP`. These are NOT extraction failures — vision succeeded; the content was just not a new trackable bet — so they are deliberately distinct from `VISION_EXTRACTION_FAILED` (mirrors `twitter-handler.js`, which already dropped the analogous vision-result/untracked_win case). The narrow `is_bet===true && bets:[]` fall-through (only way past the is_bet=false + indeterminate guards with an empty bets array) now drops as `PRE_FILTER_AI_EMPTY_RESULT` with `filter:'ai_is_bet_true_no_bets'`. Instrumentation-only — no extraction/retry/buffer behavior changed. **`GRADE_AUTOVOID_UNSCOPED`** (added 2026-06-16, audit B7 follow-up; registered `services/pipeline-events.js:81`): the terminal unsupported-sport auto-void in `gradePropWithAI` (`services/grading.js`, the sole writer of `review_status='auto_void_unscoped_bet'`) returns an `AUTO_VOIDED` sentinel that `runAutoGrade`'s if/else does not match, so the void finalized with NO `pipeline_events` and NO `grading_audit` — an empty trail that made stale pre-#110 World-Cup voids look like a phantom "separate sweep". The branch now `recordDrop`s this reason (`bets.recordDrop` call at `services/grading.js:2528`, `dropReason` literal `:2531`), gated inside `if (voided)` so it fires only when a row was actually voided (`voided = info.changes > 0`), making each unsupported-sport void queryable and DISTINCT from the no-data void (`review_status='auto_void_no_searchable_data'`, no drop emitted — sibling gap, see §Grading) and the retry-cap void (`GRADE_BACKOFF_EXHAUSTED`). The canonicalize fix itself was #110 (`canonicalizeSportForGrading` at the gate); this only closes the logging hole. **`GRADE_MANUAL_REVIEW_UNMODELED`** (added 2026-06-16): the same supported-sport gate now DIVERTS a bet to manual review (`review_status='manual_review_unmodeled_sport'`, see §Enums) instead of auto-voiding when the declared sport names a REAL intentionally-unmodeled league (KBO/KHL/NPB — `declaresAnyUnmodeledLeague`, `services/normalization.js`, ANY part of a compound). Emitted only when a row was actually parked (idempotent). DISTINCT from `GRADE_AUTOVOID_UNSCOPED` because NO grade/profit is written and `result` stays `pending` — the bet awaits a human, not a void. Null/Unknown/garbage sports still emit `GRADE_AUTOVOID_UNSCOPED`. **`GRADE_RECAP_MATCH_DEFERRED`** (added 2026-07, audit T2-01/V2; registered `services/pipeline-events.js`): the scoped recap/graphic/celebration auto-grade matcher (`services/grading.js autoGradeFromRecap` — capper scope + 7d recency are mandatory, exactly ONE in-scope match may grade) deferred to human review instead of writing a terminal grade — zero candidates, ambiguous (2+) candidates, only stale (>7d) same-capper candidates, or an unresolvable capper. Candidate bets are parked `review_status='needs_review'` and get one drop row each (plus the `bets.drop_reason` stamp); a zero-candidate deferral writes a single `bet_id`-NULL row. NOT a grade: no result/profit/bankroll write on this path. The pre-fix GLOBAL matcher (`database.js findPendingBetBySubject`, cross-capper `LIKE '%term%'` oldest-first + auto-confirm + bankroll) is deleted. **`GRADE_QUOTE_UNVERIFIED`** (added 2026-07-08): Gate 3 enforce (`QUOTE_BOUND_GRADING`) forced a grade to PENDING because the model's `evidence_quote` failed `validateEvidenceQuote` — evidence prefix `UNVERIFIED_QUOTE:` covers both variants (missing quote / not an exact normalized substring). Classified by the module-level `classifyPendingDropReason` (`services/grading.js`, extracted from `gradeSingleBet`'s `earlyReturn` closure) AHEAD of the `GRADE_PENDING_UNCLASSIFIED` catch-all these rows used to land in. Labeling only — Gate 3 behavior unchanged. **`GRADE_DATE_UNVERIFIED`** (added 2026-07-08, WC-3 follow-up): Gate 4 enforce (`DATE_BOUND_GRADING`) forced a grade to PENDING on off-date evidence — evidence prefix `OFF_DATE_EVIDENCE:` now classifies here instead of the catch-all (labeling only, Gate 4 behavior unchanged); the same batch routes the free-form LLM no-data phrasings to the EXISTING `GRADE_AI_PENDING_NO_DATA` via two anchored conservative forms — the three stems `No final score` / `No match details` / `No results`, plus (batch 2, same day) a bounded-gap form `No` + 1–7 letter-only words + `found`/`results` (digits/punctuation in the gap block the match, so score-carrying strings structurally cannot match; see the classifier comment) — while the code-templated `No final score found…` prefix keeps mapping to `GRADE_NO_SEARCH_HITS` (matched first in the chain). **`GRADE_VOID_DEFERRED_EXEMPT`** (added 2026-07-08, WC-3): a void-on-exhaustion writer (`payload.void_path` = `retry_cap`\|`no_data`\|`unscoped_sport`) found an ACTIVE `bets.sweep_exempt_until` grace window and DEFERRED its terminal void — bet stays `result='pending'`, requeued at the window's lapse (`deferVoidForGraceWindow`, `services/grading.js`). The bet is NOT dropped — non-terminal deferral marker, same semantics as `GRADE_RECAP_MATCH_DEFERRED`. Tests `tests/grace-window-void-deferral.test.js`. **`GRADE_VOID_DEFERRED_ADAPTER`** (added 2026-07-08, WC-3 residual): `RETRY_CAP_ADAPTER_EXEMPT=enforce` — the retry-cap void (`scheduleRecheckAfterDenial`) found an adapter-covered sport (`hasDeterministicAdapter`, Build-1d parity) and DEFERRED its terminal void: bet stays `result='pending'`, requeued +24h (payload `{void_path:'retry_cap', sport, denial_reason, attempts}`). Sibling of `GRADE_VOID_DEFERRED_EXEMPT` (grace window is checked FIRST); bet NOT dropped. The cap was the writer that actually voided the three WC-3 bets and, unlike the no-data void, had no adapter exemption. Tests `tests/retry-cap-adapter-exemption.test.js`.

**Grading failure-reason strings** (the `evidence` prefix on a forced-PENDING grade, NOT `pipeline_events.drop_reason` values — classified by prefix via the module-level `classifyPendingDropReason` (`services/grading.js`, extracted 2026-07-08 from the `earlyReturn` closure; pinned by `tests/terminal-state-invariant.test.js`)):
- **`UNVERIFIED_QUOTE`** — Gate 3 (`QUOTE_BOUND_GRADING=enforce`): the model's `evidence_quote` is not a verbatim substring of the evidence. Marker `GATE3_WOULD_FIRE` on `guards_failed`. **Classifies to `GRADE_QUOTE_UNVERIFIED` (2026-07-08)** — both variants (`missing evidence_quote` / `evidence_quote is not an exact substring of the evidence`) share the `UNVERIFIED_QUOTE:` prefix; pre-classifier rows sit in `GRADE_PENDING_UNCLASSIFIED`.
- **`OFF_DATE_EVIDENCE`** (added 2026-06-12, `gate4-off-date-reject`) — Gate 4 (`DATE_BOUND_GRADING=enforce`): the quote-bearing evidence record's date(s) all fall outside `[anchor−tol, anchor+tol]`. **Classifies to `GRADE_DATE_UNVERIFIED` (2026-07-08, WC-3 follow-up)** — pre-classifier rows sit in `GRADE_PENDING_UNCLASSIFIED`. Forced-PENDING evidence: `OFF_DATE_EVIDENCE: evidence dated <dates> outside <anchor>±<tol>d — forced PENDING (model claimed <STATUS>)`. Shadow/enforce both stamp `GATE4_WOULD_FIRE\|…\|reason=OFF_DATE_EVIDENCE` on `guards_failed` (zero extra rows). Pass markers `GATE4:date_ok` / `GATE4:no_date_signal` ride `guards_passed`.
...
| `selectSlipImages(images, {cap})` — **F-07 (#61)**: picks which images `handleSlipFeed` feeds to vision. Any `origin:'attachment'` present → those in order, capped at 4 (the fix); else `[images[0]]` (legacy single-image / embed-only behavior, byte-for-byte). Embed/preview thumbnails are never multiply-processed. Pure + exported. **Second consumer (Bug 1 doubling fix):** `processAggregatedMessage`'s `imageUrls` now routes through `selectSlipImages(combinedImages)` too (was `combinedImages.map(img=>img.url)`), so a TweetShift/FixTwitter relay carrying a slip card + a research card — both `origin:'embed'` — no longer parses each embed into a separate bet that `mergeBetsIntoParlay` concatenates into a DOUBLED parlay. All-embed collapses to `[images[0]]`, mirroring `handleSlipFeed`. | 590 (fn); Bug-1 call site `processAggregatedMessage` ~987 |
...
| `resolveOdds(bet)` + helpers (`americanFromDecimal`, `decimalFromAmerican`, `combineLegOdds`, `ODDS_MAX`) — odds resolution used by `normalizeBet` (replaced the old `toSafeNumber(bet.odds,-110)` + `>9999→-110` clamp). **Unknown odds resolve to `null`, NEVER a fabricated -110** (a fabricated -110 booked a winning +10372 parlay as +0.91u). Parlay order: exact leg-combination → payout-derived (`p/w`, total-return) → plausible raw; a bare `-110` on a parlay is treated as a stale default and ignored. `combineLegOdds` requires every leg be valid American (`\|o\| in [100, ODDS_MAX]`, mirrors `rawOk`) else the leg is UNPRICED → fall through (a decimal/sub-100 junk leg would otherwise collapse to a large negative). Combined parlay odds are intentionally **not** ceiling-clamped (real longshots exceed `ODDS_MAX`). Exported. **Known follow-ups (documented, not fixed here):** straight raw-odds sanity window loosened 9999→ODDS_MAX; `fromPayout` derivation is unbounded on a mis-parsed tiny wager. | helpers `americanFromDecimal` 436 / `decimalFromAmerican` 441 / `combineLegOdds` 446; `resolveOdds` 467; called in `normalizeBet` L508; export L2579 |
...
| `canFinalizeBet` | 860 |
| `scheduleRecheckAfterDenial` — gateway-denial recheck (does **not** touch attempts/state, just requeues). RETRY_CAP=15 → at cap stamps `GRADE_BACKOFF_EXHAUSTED` + VOID (cap-void UPDATE carries `AND ${GRADER_ELIGIBLE_WHERE}` (#118) + gates the `GRADE_BACKOFF_EXHAUSTED` drop on `info.changes > 0`); the cap check runs FIRST. **This cap void is the writer that actually voided the three WC-3 bets** (initially misattributed to the no-data path — BACKLOG WC-3 record correction; `sport='Soccer'` is exempt from the no-data void via `hasDeterministicAdapter`, which funnels adapter-covered no-data bets onto this denial ladder). **WC-3 grace deferral (2026-07-08):** at cap, an ACTIVE `sweep_exempt_until` window defers the void via `deferVoidForGraceWindow` (requeue at lapse + `GRADE_VOID_DEFERRED_EXEMPT` event, no terminal write); the cap void fires on the first denial after the lapse. **Adapter exemption (RETRY_CAP_ADAPTER_EXEMPT, default off — Build-1d parity, 2026-07-08):** AFTER the grace check, at cap: `enforce` + `hasDeterministicAdapter(sport)` → the void is skipped and the bet requeued `datetime('now','+24 hours')` (gated `AND result='pending' AND ${GRADER_ELIGIBLE_WHERE}`; churn bounded ~1 re-pick/day; 7-day sweeper stays the terminal backstop — `shouldAutoVoidNoData`'s exact contract) + one `GRADE_VOID_DEFERRED_ADAPTER` event gated on the requeue landing; `shadow` → one `retry_cap_adapter_shadow` would-defer row, void unchanged; `off`/unset → byte-identical pre-flag. Same inline-require + try/catch idiom as the Build-1d guard (adapter layer unavailable → falls through to the void). Tests `tests/retry-cap-adapter-exemption.test.js` (RED-proven: pre-flag the 2a/2b/2c WC-3 shapes terminally void). **Terminal-state invariant (2026-07-08):** the cap-void now writes `grading_state='done'` + `grading_next_attempt_at=NULL` in the same statement (pre-fix: `'backoff'` + `+24h` — see §Enums `grading_state`). **#124 event-aware (EVENT_AWARE_RECHECK):** `off` → unchanged flat `+minutes` write; `enforce` → `grading_next_attempt_at = datetime(?)` at `nextAttemptForEvent(event_date).nextAttemptAt` instead of the flat +30 (then `return`); `shadow` → emit one `event_aware_shadow` `would_window` row + structured log, then fall through to the unchanged flat write (behavior identical to `off`). | 1152 (`retryCapAdapterExemptMode` 1144; `RETRY_CAP` 1158; grace defer 1166; adapter-exempt block 1181–1235 (enforce requeue ~1195, drop 1208, shadow emit 1229); cap-void UPDATE 1237; event-aware block ~1287; shadow emit 1305; flat write 1310) — refreshed 2026-07-08 (RETRY_CAP_ADAPTER_EXEMPT) |
...
The grading cron's **7-Day Smart Sweeper** auto-sweeps any pending **non-prop** bet older than `SWEEP_DAYS` (7) to **VOID** (DP-01/V1, 2026-07: a sweep is evidence-free, so it must never debit bankroll or count in W-L/ROI — `services/grading.js sweepExpiredBet` writes `result='void'`, `profit_units=0`, `grade='VOID'`, no bankroll/snapshot write, and emits a `GRADING_COMPLETE` pipeline event + `grading_audit` row with `provider_used='sweeper_7d'`; pre-fix it wrote a real LOSS with bankroll impact). The `grade_reason` marker stays the `Auto-swept:` prefix, so historical LOSS sweeps and current VOID sweeps are queryable with the same `LIKE 'Auto-swept%'`. `recoverHold` backdates a recovered bet's `created_at` to the slip post time (#59), which would make it instantly sweep-eligible → false terminal grade before the grader runs. Migration **028** adds `bets.sweep_exempt_until` (TEXT, NULL default) as a self-expiring grace marker the sweeper honors.
...
| `sweepGraceUntil(betId)` — returns `sweep_exempt_until` iff set AND `datetime('now') < sweep_exempt_until` (comparison runs in SQLite, same clock/format the marker was written with), else null; reads the column fresh by id. THE exemption predicate — the sweeper AND (WC-3, 2026-07-08) all three void-on-exhaustion writers key off it | `services/grading.js:1793` — refreshed 2026-07-08 |
| **`deferVoidForGraceWindow(betId, voidPath, payload)`** — WC-3 (2026-07-08): called by the retry-cap void, `autoVoidNoSearchableData`, and the unscoped-sport auto-void BEFORE their terminal write. ACTIVE window (via `sweepGraceUntil`) → skip the void, requeue `grading_next_attempt_at = datetime(<lapse>)` (gated `AND result='pending' AND ${GRADER_ELIGIBLE_WHERE}` — a concurrently-graded or operator-parked bet is never touched; `grading_state` untouched so the retry loop re-picks after the lapse), emit one `GRADE_VOID_DEFERRED_EXEMPT` drop-event (bet NOT dropped; gated on the requeue landing). Returns the lapse or null. Sweeper deliberately unchanged. Tests `tests/grace-window-void-deferral.test.js` (RED-proven on all three paths) | `services/grading.js:1828` |
...
| `routes/admin.js` | READ-ONLY `/api/admin/*` (Phase 2a-1, `ADMIN_API_SECRET` via router-wide `router.use(adminAuth)` L40): GET `/holds` L66, `/bets` L183, `/handles` L226 (all `scraper_handles` rows → `{count, handles:[{handle,enabled,added_at,note}]}`, ordered by handle), `/logs` L243 (tails `#admin-log`); catch-all 404. Now imports `adminAuth` from `./adminAuth`. Mounted bot.js L28. **`imageUrl` per hold (#119):** each `/holds` row now carries `imageUrl: imageUrlFor(r.ingest_id)` (L156, next to `messageUrl` L155) — joined by `ingest_id` from a SEPARATE EXTRACTED-event `pipeline_events` row. The lookup `imageUrlStmt` (L98–104) is KEY-AWARE/tightened: `payload LIKE '%"imageUrl"%'` (embedded JSON-key quotes, not naive `%imageUrl%`), `ORDER BY created_at DESC, id DESC LIMIT 10`; `imageUrlFor` (L105–119) `.all()`s newest-first and returns the first row that `JSON.parse`s to a non-empty string `imageUrl`, SKIPPING rows that merely mention the word (defeats the shadowing case where a newer keyless hold's `sample` contains "imageUrl"). `null` when ingest_id falsy or no usable row. Value returned UNFILTERED (promo art / video thumbs surface too — dashboard classifies). dedup/resolved-filter/shape otherwise byte-identical. **Caveat (code comment L60–65, NOT fixed):** `messageHandler.js:1058` clips the stored url to `slice(0,120)`, so long signed Discord CDN urls arrive truncated; twitter holds + multi-image Discord holds store only `{imageCount}` (no `imageUrl`) → always `null`. **Phase A dashboard reads (#161, 2026-07-01):** GET `/leaderboard` L399 (`?sort=` whitelisted to `getLeaderboard`'s own set `total_profit_units\|roi_pct\|win_pct\|total_bets`, else silent fallback to `total_profit_units` — mirrors the function; `?limit=` clamps 1..50 default 10; envelope `{season: ACTIVE_SEASON, sort, cappers}` — `season` deliberate, this is the season-scoped truth surface); GET `/drops` L443 (`recordDrop()` rows via **`event_type='DROP'`** — drop rows ride MANY stages (`DROPPED`, `GRADING_DROPPED`, pass-throughs like `GRADING_AI` — `services/grading.js:3185` via `bets.recordDrop`), so a stage-list filter would silently omit; hold rows are excluded by construction (their `drop_reason` COLUMN is always NULL — `recordStage` hardcodes it; the hold reason rides payload JSON); `?hours=` clamps 1..168 default 24, cutoff computed in JS as epoch-SECONDS and bound as a param — `created_at` INTEGER quirk; `?reason=` format-validated `^[A-Z0-9_]+$` → 400, enum NOT hardcoded (DROP_REASONS grows); `?limit=` clamps 1..200 default 50 caps `drops` only; envelope `{since, counts (window-wide, grouped desc), drops (newest first)}`, `payload` raw TEXT; both queries served by `idx_pipeline_events_event_type_created`, **mig 031** — without it they are full-table scans, synchronous on the bot's event loop); GET `/grader-health` L497 (envelope `{pending, grading24h, backends24h}`: `pending` = total / `oldestCreatedAt` / `byReviewStatus` FROM bets `result='pending'`; `grading24h` = attempts / `distinctBets` / `byProvider` / `byFinalStatus` FROM `grading_audit` `timestamp >= (nowSec-86400)*1000` — **MILLIS**; `backends24h` grouped backend+status FROM `search_backend_calls` `ts >=` same MILLIS cutoff — `ts` is `Date.now()` per `recordBackendCall` (`grading.js:2288`), mig 023 declares only `INTEGER`). All three: lazy-required db, adminAuth via the router-wide `use`, SELECT-only, catch → 500 `Internal error`. Tests `tests/admin-read-endpoints.test.js`. |
...
### services/bets.js
BetService — grading-side lifecycle writes (`source_type='grading'`; sole prod importer is `services/grading.js`). `transitionTo` (L107) = one `pipeline_events` row per call (+ `bets.drop_reason` stamp when `eventType='DROP'`); `recordDrop` (L173) is the DROP convenience wrapper. Fire-and-forget contract: nothing here throws up the call stack.

| What | Line(s) |
| --- | --- |
| `readGradingAttemptsForKey(betId)` — Stage 2 idempotency: `bets.grading_attempts` read at write time (bets PK is `id`). Synthetic parlay-leg ids `<parent>-leg<N>` (`services/grading.js` gradeParlay) have no bets row → strips the suffix and reads the PARENT (its `grading_attempts` is the counter for the whole pass; the FULL leg id still goes in the key so sibling legs stay distinct). No row either way → `null` → key skipped → write behaves as pre-032. Never throws. | 53 |
| `computeGradingIdemKey({betId,stage,eventType,dropReason})` — key for a grading-side write or `null`. **DROP events ONLY** (deliberate): non-DROP grading writes are per-decision telemetry where same-attempt repeats ARE the signal — `event_aware_shadow` fires once per cron poll on the PRE-claim path (attempts unchanged between polls), so keying it would collapse the poll-frequency measurement. Drop analytics (`GET /api/admin/drops`) reads `event_type='DROP'`, exactly the deduped population. `off` → key not computed at all. | 80 |
| `transitionTo` passes `idempotencyKey` into `pipelineEvents.writeRow` (mode-dependent behavior lives THERE, not here) | 152 |

### services/pipeline-events.js
| What | Line(s) |
| --- | --- |
| Stage enum | 18 (`STAGES`); `EVENT_TYPES` L46; `DROP_REASONS` L60 |
| **Stage 2 idempotency (mig 032, `PIPELINE_IDEM_MODE`)** — `resolvePipelineIdemMode` L132 (strict `off\|shadow\|enforce`, unset/garbage → `off`; read PER CALL like `eventAwareRecheckMode`, not at load like OCR wiring, so ops flip without restart); `deriveIdempotencyKey` L155 (pure, `gradv1\|betId\|attempts\|stage\|eventType\|dropReason`); `shadowDuplicateExists` L179 (shadow probe via `json_valid(payload) AND json_extract(payload,'$.idem_key')`, bounded `event_type` + `created_at >= now-7d` so it rides mig 031's `idx_pipeline_events_event_type_created` — NOT a full scan; probe failure → treated as no-dup, write proceeds); `writeRow` mode branch L289 (shadow: column NULL + payload `idem_key`/`idem_would_reject` FIRST in the object — safeJson slices at 4000 chars, trailing keys are the truncation casualties — + console marker, WRITES ANYWAY; enforce: column populated + payload mirror, duplicate trips the partial unique index → caught, logged `idem enforce rejected duplicate`, swallowed — NEVER thrown). Keys arrive only from `services/bets.js` (grading DROPs); every other caller leaves `idempotencyKey` undefined → block inert. | 132–205, 276 |
...
| `retry_cap_adapter_shadow` event_type (RETRY_CAP_ADAPTER_EXEMPT) — registered in `EVENT_TYPES`; shadow-only would-fire telemetry on stage `GRADING_ENTER`, one row per adapter-covered bet the retry-cap void WOULD defer under enforce (payload `{kind:'would_defer', void_path:'retry_cap', sport, denial_reason, attempts}`). Additive/observational, never gates behavior (the cap void proceeds unchanged in shadow; enforce requeues +24h + `GRADE_VOID_DEFERRED_ADAPTER` and emits none of these); NOT in `EXPECTED_STAGES`. Written inline by `scheduleRecheckAfterDenial` (`services/grading.js:1229`). | 59 |
...
| **Season scoping → `docs/SEASON-RESET.md`.** `getCapperStats`/`getLeaderboard` gate on `AND b.season = ? = ACTIVE_SEASON` (bump = fresh-slate reporting, no delete). **4 all-time bypass sites that ignore season:** `!leaderboard` (`bot.js:322`), `!reset_season` podium (`bot.js:467`), `dashboardSummary` (`database.js:267`), `getCapperAnalytics` (`database.js:1079`). | see `docs/SEASON-RESET.md` |
| `findPendingBetBySubject` | 1032 |
| `getPendingBets` — the autograder + 7-day sweeper's ONLY source. NULL-safe `needs_review`/`manual_review_unmodeled_sport` exclusion via `AND (b.review_status IS NULL OR b.review_status NOT IN (...))` (#89/#113), keyed on `GRADER_HIDDEN_REVIEW_STATUSES`. Both selector paths filter `result = 'pending'` (since P0), so terminal-result rows are never picked regardless of `grading_state` — that skip is made VISIBLE by the one-time-per-process `logTerminalStateDriftOnce` (first call): logs the count of terminal-result rows whose `grading_state` is still `ready`/`backoff`/`quarantined` (`countTerminalStateDrift`, exported for tests; 2026-07-08 terminal-state invariant) | 786 (`GRADER_HIDDEN_REVIEW_STATUSES` 733; review-status guard clause 798; drift-log block 735–784, `countTerminalStateDrift` 752, `logTerminalStateDriftOnce` 766) — refreshed 2026-07-08 |
...
### services/slateResplit.js (mixed-sport recap-SHEET re-split, gated)

> Deterministic post-parse safety net for the mixed soccer/MMA slate defect (bet `8436c0c7`, @Bobby_tracker). A recap slate is a LIST of independently-staked picks; Vision is told to split a multi-sport SHEET into per-pick straights (`GEMMA_SLIP_PROMPT` / `parseBetText`, ai.js:1073) but on a genuinely mixed slate returns **one `bet_type:parlay`, N legs, the SLATE's DOMINANT sport** (verified `pipeline_events` `ingest_id=twit_2068786490960945493`: PARSED legCount:8 betType:parlay sport:Soccer). `twitter-handler` stages `bets[0]` verbatim, so every leg (incl. the MMA fighters) inherits the dominant sport and the per-pick stakes are lost. The all-MMA sibling tweet parses as a UFC parlay — same picks, correct sport — proving sport is decided **once per slate**, never per leg. The downstream split of that parlay into per-pick straights is a **regrade/manual op** (out of scope of this module). Gated by `SLATE_RESPLIT_MODE` (see §Env vars); **default off = byte-identical live behavior**. Mirrors `ocrFirstWiring.js` conventions (off/shadow/cutover, injectable deps, never-throws). Tests: `tests/slate-resplit.test.js` (68).

| What | Line(s) |
| --- | --- |
| `MODE` / `resolveMode(raw)` — `SLATE_RESPLIT_MODE` → off\|shadow\|cutover (unset→off) | `MODE` 55 |
| `MMA_MARKERS` — combat-only market signals (`\bitd\b`, `by (ko\|tko\|submission)`, `by …decision`, `wins by …`, `fight goes the distance`, `ufc`) → deterministic per-pick MMA. Hardened vs cross-domain/prose collisions (no bare `round over/under`, decision types require `by`) | 62 |
| `parseUnits` / `parseOdds` / `stripStake` / `splitSegments` — per-pick stake (clamp 0.01–100), American odds, stake-strip, pipe/newline split | 86 / 93 / 103 / 108 |
| `inferPickSport(text, fallbackSport, deps)` — MMA marker → `inferLegSport` (modeled team) → `descNamesNationalTeam` (nation→Soccer) → INHERIT vision sport (low-confidence). Bare `<fighter> ML` lands on inherit by design (no roster) | 137 |
| `parsePick(segment, fallbackSport, deps)` — a segment is a sheet pick only if it carries its OWN stake token (the per-pick-stake discriminator) | 154 |
| `detectSheet({pick, rawText, deps})` — `isSheet` ⇔ multi-leg vision bet AND ≥2 independently-staked pick segments | 169 |
| `applySlateResplit({...})` — off: no-op; shadow: emit `slate_resplit_shadow`, NEVER act; cutover: emit `slate_resplit_used`, return the per-pick straights. Never throws | 238 |
| twitter-handler wiring — require L10; sheet branch after the ladder branch, before the F-12 normal path (`if (slateResplit.MODE !== 'off')` → cutover loops per-pick `createBetWithLegs` straights with per-step F-12 dedup, mirrors the ladder branch) | `services/twitter-handler.js` |
...
| 032 | `pipeline_events.idempotency_key` (nullable TEXT) + partial unique index `idx_pipeline_events_idem_key` (`WHERE idempotency_key IS NOT NULL`) — Stage 2 grading-side DROP dedup, additive-only (existing rows/defaults untouched; `created_at` stays epoch-sec). Column populated only under `PIPELINE_IDEM_MODE=enforce` |
...
| RETRY_CAP_ADAPTER_EXEMPT | `scheduleRecheckAfterDenial` cap block (`retryCapAdapterExemptMode`, `services/grading.js:1144`) — read per call | unset/anything-else → `off` = byte-identical: an adapter-covered sport at RETRY_CAP still terminally cap-voids (the residual WC-3 hole). `shadow` → measure only: one `retry_cap_adapter_shadow` `would_defer` row per adapter-covered cap void, void unchanged. `enforce` → Build-1d parity: `hasDeterministicAdapter(sport)` at cap skips the void, requeues +24h + one `GRADE_VOID_DEFERRED_ADAPTER` event (grace-window deferral #191 still checked FIRST; 7-day sweeper stays the terminal backstop). **Default off; shadow-first measurement precedes the enforce flip** — see docs/FLAG-FLIPS.md + BACKLOG WC-3 |
...
| PIPELINE_IDEM_MODE | Stage 2 grading-side DROP dedup (`resolvePipelineIdemMode`, `services/pipeline-events.js:132` — read per call; key computed in `services/bets.js computeGradingIdemKey`, applied in `writeRow`) | unset/anything-else → **`off`** = key not computed, byte-identical live behavior (deploying with the flag unset is a no-op). `shadow` → compute key, probe recent rows via payload `$.idem_key`, WRITE ANYWAY with the would-reject marker (`idem_would_reject:true` + console line) — the `idempotency_key` COLUMN stays NULL in shadow. `enforce` → populate the column; a same-`(bet, attempt, stage, event_type, drop_reason)` duplicate trips mig 032's partial unique index and is silently swallowed (logged, never thrown). Cross-attempt repeats always survive (`grading_attempts` is in the key). Shadow-review SQL: `SELECT COUNT(*) FROM pipeline_events WHERE event_type='DROP' AND json_extract(payload,'$.idem_would_reject')=1 AND created_at >= strftime('%s','now')-86400*7` |
| SLATE_RESPLIT_MODE | mixed-sport recap-SHEET re-split gate (`services/slateResplit.js`, `MODE` at load; wired in `services/twitter-handler.js` after the vision parse, before the F-12 normal path) | unset/anything-else → **`off`** = no-op, byte-identical live behavior (the block is guarded by `MODE!=='off'`; merging changes nothing). `shadow` → emits one `slate_resplit_shadow` pipeline_events row per multi-leg vision parlay whose raw tweet text is a delimited list of independently-staked picks (would-split decision + per-pick sport/units sample) and **never re-splits** (parlay stages as today). `cutover` → re-splits the sheet into per-pick straights (per-pick units from the stake token; sport = ITD/finish→MMA, national team→Soccer, modeled team→its league via `inferLegSport`, else INHERIT the vision sport flagged low-confidence — a bare `<fighter> ML` is NOT force-guessed to MMA without a roster) and emits `slate_resplit_used`. Mirrors `OCR_FIRST_MODE`. |
