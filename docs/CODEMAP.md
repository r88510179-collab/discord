...
| `soccer_grade_shadow` event_type (Build 1) — registered in `EVENT_TYPES`; shadow-only would-fire telemetry on stage `GRADING_ENTER`, one row per soccer bet/leg the adapter settles (`would_status` WIN/LOSS/PUSH/VOID) or audits (`reason` match_not_final/no_match_found). Payload `{bet_id, would_status, reason, evidence, source:'espn_soccer', match_id, slug:'fifa.world', desc_or_leg}`. **Build 1b:** the SAME emit covers PLAYER-PROP would-verdicts (a prop WIN/LOSS/VOID emits via the existing `shouldEmitSoccerShadow`); prop resolution fall-throughs (`player_not_found`/`no_unique_player`/`player_stat_missing`/`keyevents_incomplete`) stay SILENT, like the match-level `unsupported_market_soccer` — `emitSoccerShadow`/`shouldEmitSoccerShadow` are UNCHANGED. Additive/observational, NO grade write (shadow returns fall-through); enforce returns the resolved status and emits none. NOT in `EXPECTED_STAGES`. Written by `emitSoccerShadow` (`services/sportsdata/index.js`). | 44 |
