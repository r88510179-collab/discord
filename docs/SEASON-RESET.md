...
## Known footgun (RESOLVED 2026-07-01)
Two migrations formerly shared number 006, both `ALTER TABLE bets ADD COLUMN season`: `006_add_season_to_bets.sql` and `006_add_season_column.sql`. The migrator tracks by **filename**, not by number (`services/migrator.js:24` PK, `:42-44` sorted discovery), so **both** ran: alphabetically `006_add_season_column.sql` applied the column first, then `006_add_season_to_bets.sql` threw `duplicate column name`. What prevented a hard boot failure was the migrator's own duplicate-column tolerance (`migrator.js:61-71`, which swallows that error and marks the file applied) — **not** the `database.js:62` boot guard. That boot guard runs *after* `runMigrations(db)` (`database.js:19`), so by the time it checks, the column already exists and `if (!betCols.includes('season'))` is false: an inert no-op for this collision (it's a belt-and-suspenders fallback for a fresh/legacy DB, not the masker). **Resolved:** `006_add_season_to_bets.sql` was deleted (the `_column` superset survives, incl. `idx_bets_capper_season`); the `_to_bets` row already recorded in `schema_migrations` on live DBs is inert because the migrator never asserts recorded filenames still exist on disk. Fresh DBs run one clean 006.
