...
- Each leg MUST include: description, odds (or null), team (or player name), line, type (spread/moneyline/total/prop), match (the "<TeamA> @ <TeamB>" game line, or null).
...
- MATCH RESULT / DRAW SELECTION (CRITICAL): For a 3-way "Match Result" / moneyline market (common in soccer), each leg MUST name the SELECTION being backed — a specific team OR the draw — never just the matchup. Book slips show the selection as a bold label ("Tie", "Draw", or a team name) sitting above/beside the two team names and a "Match Result" market line. Capture that label: if it reads "Tie" or "Draw", the pick is the DRAW -> description "Draw - <TeamA> vs <TeamB>", team "Draw", type "moneyline". If it names a team, the pick is that team -> description "<Team> ML", team "<Team>", type "moneyline". NEVER output a bare "<TeamA> - <TeamB>" (matchup only) with no selection — a matchup alone is ungradeable. Do NOT assume Draw; use whichever selection the slip actually shows.
- MATCH BINDING (CRITICAL): Book/exchange slips print the game under each leg as a matchup line, usually "<TeamA> @ <TeamB>" or "<TeamA> vs <TeamB>" directly beneath the pick. You MUST read that line and bind it to the leg: set the leg's "match" field to the exact "<TeamA> @ <TeamB>" string, and for team/total/spread legs that name no player, APPEND it to the description as "<pick> (<TeamA> @ <TeamB>)" so the leg is never stored as a bare market. This is REQUIRED for market-type legs with no player name (e.g. "Total Goals: Over 1.5", "Both Teams To Score: Yes", a bare spread/total) — a market with no bound match is ungradeable. If the matchup line is genuinely absent from the image, leave "match" null and keep the pick as-is; do NOT invent a matchup.
...
- For parlays, ALWAYS populate the "legs" array. Each leg MUST have: description, odds (or null), team (or player name), line (spread/total/stat line), type (spread/moneyline/total/prop), match (the "<TeamA> @ <TeamB>" game line, or null).
- MATCH RESULT / DRAW SELECTION (CRITICAL): For a 3-way "Match Result" / moneyline market (common in soccer), each leg MUST name the SELECTION being backed — a specific team OR the draw — never just the matchup. Book slips show the selection as a bold label ("Tie", "Draw", or a team name) sitting above/beside the two team names and a "Match Result" market line. Capture that label: if it reads "Tie" or "Draw", the pick is the DRAW -> description "Draw - <TeamA> vs <TeamB>", team "Draw", type "moneyline". If it names a team, the pick is that team -> description "<Team> ML", team "<Team>", type "moneyline". NEVER output a bare "<TeamA> - <TeamB>" (matchup only) with no selection — a matchup alone is ungradeable. Do NOT assume Draw; use whichever selection the slip actually shows.
- MATCH BINDING (CRITICAL): Book/exchange slips print the game under each leg as a matchup line, usually "<TeamA> @ <TeamB>" or "<TeamA> vs <TeamB>" directly beneath the pick. You MUST read that line and bind it to the leg: set the leg's "match" field to the exact "<TeamA> @ <TeamB>" string, and for team/total/spread legs that name no player, APPEND it to the description as "<pick> (<TeamA> @ <TeamB>)" so the leg is never stored as a bare market. This is REQUIRED for market-type legs with no player name (e.g. "Total Goals: Over 1.5", "Both Teams To Score: Yes", a bare spread/total) — a market with no bound match is ungradeable. If the matchup line is genuinely absent from the image, leave "match" null and keep the pick as-is; do NOT invent a matchup.
