...
const EVENT_TYPES = [
  ...
  'retry_cap_adapter_shadow', // shadow (RETRY_CAP_ADAPTER_EXEMPT): retry-cap adapter-exemption would-fire measurement — one row per adapter-covered below-ceiling bet the cap void WOULD defer under enforce (kind=would_defer) on stage 'GRADING_ENTER', emitted by services/grading.js scheduleRecheckAfterDenial INSIDE the voidTx gated on the void landing, so the measured population exactly matches enforce's deferral set (same WHERE guards; parked/settled no-ops emit nothing). Additive/observational, never gates behavior (enforce requeues +24h + GRADE_VOID_DEFERRED_ADAPTER instead); not in EXPECTED_STAGES.
  'event_date_sanity_rejected', // shadow/enforce (EVENT_DATE_SANITY_MODE): the write-gate sanity guard (services/eventDate.js applyEventDateSanityGuard) NULLed an implausible extracted event_date at bet creation — one row per rejection on stage 'STAGED', emitted by createBet (services/database.js emitEventDateSanityRejected) with the bet's own ingest identity (disc_/twit_ from source_message_id/source_tweet_id; bets with neither source ref are skipped). payload {where:'createBet', source, rejected_value, gap_days, raw, min_gap_days, max_gap_days, mode}. ATYPICAL gating: the guard's NULLing is ALWAYS-ON (#153/#154) — the flag gates only this telemetry, so off = warn-log-only (byte-identical) and enforce ≡ shadow. Additive/observational; NOT in pipelineHealth.EXPECTED_STAGES.
  'prop_parse_v2_shadow',// shadow (PROP_PARSE_V2_MODE): MLB prop-parser v2 would-parse measurement — one row per DISTINCT description the v1 grammar cannot USABLY parse (null stat, or a player field carrying separator/verbose debris that can never hit a box score) but the flag-gated v2 grammar can (payload pattern: 'v1ext'|'D'|'verbose'|'bare_hr'), emitted by services/sportsdata/mlb.js parsePlayerProp on stage 'GRADING_ENTER' with sourceType 'grading' (the parser has no ingest identity; betId NULL — the in-process dedup keys on the FULL trimmed description and payload desc is that key's 90-char prefix, so two 90+-char shapes sharing a prefix can write two rows with identical desc; per-process description dedup keeps it one row per shape across the 2-3 parser hits per grading pass + rechecks). payload {where, desc<=90, pattern, player, stat, threshold, direction}. Measurement-only, routing byte-identical to off (the v1 result is returned unchanged); enforce parses with v2 instead and emits nothing; not in EXPECTED_STAGES.
  'reaper_shadow',       // shadow (REAPER_MODE): exhaustion-terminal routing would-fire measurement — one row per bet an exhaustion writer (retry-cap void / no-data void / unscoped-sport void) WOULD route to review_status='needs_review' under enforce (kind=would_route, payload {writer, sport, attempts, would_reason: GRADE_EXHAUSTED_*}), emitted INSIDE each writer gated on its void actually landing so the measured population exactly matches enforce's routing set; PLUS one row per zombie-sweep candidate (writer='zombie_sweep' — no void exists there, so the row is gated only on the candidate passing every skip). Stage 'GRADING_ENTER'. Additive/observational, never gates behavior (enforce parks the bet + emits the GRADE_EXHAUSTED_* drop instead); not in EXPECTED_STAGES.
];
const DROP_REASONS = [
  'DUPLICATE_IMAGE',
  ...
  'GRADE_QUOTE_UNVERIFIED',         // Gate 3 enforce (QUOTE_BOUND_GRADING) forced PENDING: the model's evidence_quote failed validateEvidenceQuote — missing, or not an exact (normalized) substring of the evidence. Evidence prefix `UNVERIFIED_QUOTE:` (services/grading.js gradeSingleBet → classifyPendingDropReason). Predates-classifier rows landed in GRADE_PENDING_UNCLASSIFIED. Labeling only — Gate 3 behavior unchanged. 2026-07-08.
  'GRADE_DATE_UNVERIFIED',          // Gate 4 enforce (DATE_BOUND_GRADING) forced PENDING: the quote-bearing evidence record's dates all fall outside the bet's anchor±tolerance window. Evidence prefix `OFF_DATE_EVIDENCE:` (services/grading.js gradeSingleBet → classifyPendingDropReason). Pre-classifier rows landed in GRADE_PENDING_UNCLASSIFIED. Labeling only — Gate 4 behavior unchanged. 2026-07-08.
  'GRADE_VOID_DEFERRED_EXEMPT',     // a void-on-exhaustion writer (payload.void_path: 'retry_cap' | 'no_data' | 'unscoped_sport') found an ACTIVE bets.sweep_exempt_until grace window (mig 028) and DEFERRED its terminal void instead of writing it: the bet stays result='pending' and is requeued at the window's lapse (grading.js deferVoidForGraceWindow). The bet is NOT dropped — non-terminal deferral marker, same semantics as GRADE_RECAP_MATCH_DEFERRED. Closes the WC-3 hole (2026-07-08: three recovered HRB World Cup bets no-data-voided INSIDE their grace window; the window previously only guarded the 7-Day Sweeper).
  'GRADE_VOID_DEFERRED_ADAPTER',    // RETRY_CAP_ADAPTER_EXEMPT=enforce: the retry-cap void (scheduleRecheckAfterDenial) found an adapter-covered sport (hasDeterministicAdapter — Build-1d parity) BELOW the deferral ceiling (RETRY_CAP+RETRY_CAP_EXEMPT_EXTRA=19) and DEFERRED its terminal void: the bet stays result='pending' and is requeued +24h (payload: {void_path:'retry_cap', sport, denial_reason, attempts}). At/above the ceiling the cap void fires as pre-flag — the cap is the terminal guarantee for its pending-legs-parlay population, which the 7-Day Sweeper structurally CANNOT settle (sweepExpiredBet's terminal write is denied pending_legs by the same canFinalizeBet gate and lands back in this fn). The bet is NOT dropped — non-terminal deferral marker, sibling of GRADE_VOID_DEFERRED_EXEMPT (which keys on the grace window and is checked FIRST). Closes the WC-3 residual: the cap was the writer that actually voided the three WC-3 bets, and unlike the no-data void it had no adapter exemption. 2026-07-08.
  'GRADE_EXHAUSTED_ADAPTER_REVIEW', // REAPER_MODE=enforce: an exhaustion terminal (payload.writer: 'retry_cap' | 'no_data' | 'unscoped_sport' | 'zombie_sweep') routed an ADAPTER-COVERED bet (hasDeterministicAdapter) to review_status='needs_review' INSTEAD of writing a terminal void — result stays 'pending', grading_state='done' (the #113 divert contract: grader + sweeper never re-pick it; approveBet re-arms). Fires only AFTER the #191 grace deferral and #193's adapter deferral runway are spent. Operator-ratified policy (WC-3: 3/3 exhaustion-voids were wrong): "search/adapter failed N times" is the system's blindness, not proof the event didn't settle — void is operator-only on these paths. payload: {writer, sport, attempts, adapter_covered:true}. 2026-07-08.
  'GRADE_EXHAUSTED_NO_SOURCE_REVIEW', // REAPER_MODE=enforce: same routing as GRADE_EXHAUSTED_ADAPTER_REVIEW but for a SEARCH-ONLY bet (no deterministic adapter covers its sport) — split so "adapter should have settled this" is queryable apart from "no source exists". Same payload shape (adapter_covered:false). 2026-07-08.
