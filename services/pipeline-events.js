...
const EVENT_TYPES = [
  ...
  'retry_cap_adapter_shadow', // shadow (RETRY_CAP_ADAPTER_EXEMPT): retry-cap adapter-exemption would-fire measurement — one row per adapter-covered bet the cap void WOULD defer under enforce (kind=would_defer) on stage 'GRADING_ENTER', emitted by services/grading.js scheduleRecheckAfterDenial just before the (unchanged) cap void. Additive/observational, never gates behavior (enforce requeues +24h + GRADE_VOID_DEFERRED_ADAPTER instead); not in EXPECTED_STAGES.
];
const DROP_REASONS = [
  'DUPLICATE_IMAGE',
  ...
  'GRADE_VOID_DEFERRED_ADAPTER',    // RETRY_CAP_ADAPTER_EXEMPT=enforce: the retry-cap void (scheduleRecheckAfterDenial) found an adapter-covered sport (hasDeterministicAdapter — Build-1d parity) and DEFERRED its terminal void: the bet stays result='pending' and is requeued +24h (payload: {void_path:'retry_cap', sport, denial_reason, attempts}). The bet is NOT dropped — non-terminal deferral marker, sibling of GRADE_VOID_DEFERRED_EXEMPT (which keys on the grace window and is checked FIRST). Closes the WC-3 residual: the cap was the writer that actually voided the three WC-3 bets, and unlike the no-data void it had no adapter exemption. 2026-07-08.
